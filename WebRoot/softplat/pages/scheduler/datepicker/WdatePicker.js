...
function WdatePicker(el,dateFmt,showTime,skin){
	...
	if(c==undefined){c=dpcfg.maxDate;}
	if(c.substring(0,1)=='$'){
		c=document.getElementById(c.substring(1));
		if(c){
			if(c.getAttribute("REALVALUE")==null||c.getAttribute("REALVALUE")==""){
				c=c.value;}else{c=c.getAttribute("REALVALUE");
			}
		}else{
			c=null
		};
	...
	if(this.top.document.dateDiv&&this.top.document.dateDiv.obj.eCont==this.eCont){
		$d=this.top.document.dateDiv;}else{this.top.document.dateDiv=null;
	}
	...
	if(this.eCont.value!=''&&this.eCont.getAttribute("REALVALUE")==null&&this.l(this.eCont.value)){this.am();}
	this.bt=this.year;
	this.bq=this.month;
	this.bs=this.date;
		...
		for(var i=this.top.document.styleSheets.length-1;i>=0;i--){
			...
			if(tempStyle.href.substring(tempStyle.href.lastIndexOf('/')+1).toLowerCase()=='datepicker.css'){
				if(tempStyle.href.toLowerCase()==this.cssPath.toLowerCase()){
					ay=false;
					tempStyle.disabled=false;continue;
				}else{
...
WdatePicker.prototype.u=function(){
	...
	if((this.year*100+this.month*1)<(this.minDate.year*100+this.minDate.month*1)){
		...
		e=this.year;tempMonth=this.month;
	}
	...
	var ag='';var bu='';var bi=((e==this.aa)&&(tempMonth==this.p));
...
WdatePicker.prototype.pickDate=function(Y,M,D,h,m,s){
	...
	this.am(Y,M,D,h,m,s);$d.obj._markValue(true);
...
WdatePicker.prototype._markValue=function(bValue){
	if(bValue){
		...
		while(true){
			switch(bf){
				case 0:if(!confirm(dpcfg.errAlertMsg)){bf=2;continue;}
				case 1:if(this.eCont.getAttribute("REALVALUE")){
							this.l(this.eCont.getAttribute("REALVALUE"))}
						else{this.eCont.value="";}
						this.eCont.className=this.eCont.className.replace(/ WdateFmtErr/,'');
						break;
				case 2:this.eCont.className=this.eCont.className.replace(/ WdateFmtErr/,'');
...
if(navigator.product=='Gecko'){
	...
	Document.prototype.createStyleSheet=function(cssPath){
		...
		style.href=cssPath;style.rel='stylesheet';
	...
	HTMLElement.prototype.__defineSetter__("outerHTML",function(sHTML){
		...
		this.parentNode.replaceChild(df,this);return sHTML;
	});
	HTMLElement.prototype.__defineGetter__("outerHTML",function(){
		var attr;var attrs=this.attributes;
	...
	HTMLElement.prototype.getBoundingClientRect=function(){
		var obj=this;var top=obj.offsetTop;var left=obj.offsetLeft;
		var right=obj.offsetWidth;var bottom=obj.offsetHeight;
